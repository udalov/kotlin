...
private class IrDeserializerPrinter(
    ...
    fun buildDeclaration(): String {
        ...
        sb.appendLine("${prefix}class ${name}ProtoReader(source: ByteArray) : ProtoReader(source) {")
    ...
    private fun String.escape(): String {
        val result = this.split('_').fold("") { acc, c ->
            if (c.isEmpty()) acc else {
                if (acc.isEmpty()) {
                    c[0].lowercaseChar() + c.substring(1)
                } else {
                    acc + c[0].uppercaseChar() + c.substring(1)
                }
            }
        }
    ...
    private fun StringBuilder.addAbstractFactory(p: Proto) {
        ...
        when (p) {
            is Proto.Enum -> {
                ...
                appendLine("    ${maybeAbstract}fun create${p.name}(index: Int): ${p.name.ktType}${impl}")
            }
            is Proto.Message -> {
    ...
    private fun StringBuilder.addMessage(m: Proto.Message) {
        ...
        if (exposedFields.isNotEmpty()) {
            for (f in exposedFields) {
                ...
                f.type.zeroValue?.let {
                    appendLine("    protected var $name: ${f.type.ktType} = ${it}")
                } ?: appendLine("    protected var $name: ${f.type.ktType}? = null")
            }

            appendLine()
        }

        appendLine("    open fun read${m.name}(): ${m.name.ktType} {")

        val iterations = allFields.groupBy { if (it in exposedFields) -1 else it.order }
            .entries.sortedBy { (k, _) -> k }.map { (_, v) -> v }.filter { !it.isEmpty() }
        ...
        allFields.forEach { f ->
            ...
            appendLine("        var ${getName(f)}: $type = $initExpression")

            if (fieldToIteration[f] != 0 && typeMap[f.type] is Proto.Message) {
                delayedReads += f
                if (f.kind == FieldKind.REPEATED) {
                    appendLine("        var ${getName(f)}OffsetList: MutableList<Int> = arrayListOf()")
                } else {
                    appendLine("        var ${getName(f)}Offset: Int = -1")
                }
            }
        }


        m.fields.inlined().filterIsInstance<MessageEntry.OneOf>().singleOrNull()?.let { of ->
            appendLine("        var oneOfIndex: Int = -1")
            of
        }


        fun readFields(shift: String, fields: List<MessageEntry.Field>) {
            appendLine("${shift}while (hasData) {")
            appendLine("${shift}    readField { fieldNumber, type -> ")
            appendLine("${shift}        when (fieldNumber) {")

            val indent = "${shift}            "
            fields.forEach { f ->
                if (f.isInline) {
                    appendLine("${indent}${f.index} -> readWithLength {")
                    ...
                    if (f.kind == FieldKind.REPEATED) {
                        if (f in delayedReads) {
                            appendLine("${indent}${f.index} -> {")
                            appendLine("${indent}    ${getName(f)}OffsetList.add(offset)")
                            appendLine("${indent}    skip(type)")
                            appendLine("${indent}}")
                        } else {
                            appendLine("${indent}${f.index} -> ${getName(f)}.add($readExpression)")
                        }
                    } else if (f.kind == FieldKind.ONE_OF) {
                        appendLine("${indent}${f.index} -> {")
                        if (f in delayedReads) {
                            appendLine("${indent}    ${getName(f)}Offset = offset")
                            appendLine("${indent}    skip(type)")
                        } else {
                            appendLine("${indent}    ${getName(f)} = $readExpression")
                        }
                        appendLine("${indent}    oneOfIndex = ${f.index}")
                        appendLine("${indent}}")
                    } else {
                        if (f in delayedReads) {
                            appendLine("${indent}${f.index} -> {")
                            ...
                            appendLine("${indent}${f.index} -> ${getName(f)} = $readExpression")
                        }
                    }
                }
            }

            appendLine("${shift}            else -> skip(type)")
            appendLine("${shift}        }")
            appendLine("${shift}    }")
            appendLine("${shift}}")
        }
        ...
        fun invokeCreateWithOneOf(index: Int, fields: List<MessageEntry.Field>) {
            ...
            if (!hasOneOf) {
                if (index != lastIteration || hasExposed) {
                    appendLine("        val p${index} = ${invokeCreate(index, suffixPrefix, fields)}")
                } else {
                    appendLine("        ${invokeCreate(index, suffixPrefix, fields)}")
                }
            } else {
                val fp = mutableListOf<MessageEntry.Field>()
                val fs = mutableListOf<MessageEntry.Field>()
                ...
                if (index != lastIteration || hasExposed) {
                    appendLine("        val p${index} = when (oneOfIndex) {")
                } else {
                    appendLine("        when (oneOfIndex) {")
                }

                for (f in mf) {
                    appendLine("            ${f.index} -> ${invokeCreate(index, "${suffixPrefix}_${f.name.escape()}", fp + f + fs)}")
                }
                appendLine("            else -> error(\"Incorrect oneOf index: \" + oneOfIndex)")
                appendLine("        }")
            }
        }
        ...
        if (!hasExposed) {
            ...
            if (lastIteration != 0) appendLine()
        } else {
            for (f in exposedFields) {
                appendLine("        val old${exposedName[f]} = ${exposedName[f]}")
                appendLine("        ${exposedName[f]} = ${getName(f)}")
            }
        }
        ...
        for (i in 1 until iterations.size) {
            ...
            for (f in fields) {
                if (f.kind == FieldKind.REPEATED) {
                    appendLine("        for (o in ${getName(f)}OffsetList) {")
                    appendLine("            ${getName(f)}.add(delayed(o) { ${f.type.toReaderInvocation()} })")
                    appendLine("        }")
                } else {
                    appendLine("        if (${getName(f)}Offset != -1) {")
                    appendLine("            ${getName(f)} = delayed(${getName(f)}Offset) { ${f.type.toReaderInvocation()} }")
                    appendLine("        }")
                }
            }
            ...
            if (lastIteration != index) appendLine()
        }

        for (f in exposedFields) {
            appendLine("        ${exposedName[f]} = old${exposedName[f]}")
        }

        if (hasExposed) {
            appendLine("        return p${lastIteration}")
        }
