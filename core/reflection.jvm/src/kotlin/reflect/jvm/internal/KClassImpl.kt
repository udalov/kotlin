...
import org.jetbrains.kotlin.metadata.jvm.deserialization.QuickJvmNameResolver
...
internal class KClassImpl<T : Any>(
    ...
    inner class Data : KDeclarationContainerImpl.Data() {
        ...
        val rawProtoBufQuick: Pair<QuickJvmNameResolver, QuickProtoBuf.Class>? by ReflectProperties.lazySoft {
            jClass.getAnnotation(Metadata::class.java)?.let { metadata ->
                JvmProtoBufUtil.quickReadClassDataFrom(metadata.data1, metadata.data2)
            }
        }
    ...
    override val members: Collection<KCallable<*>>
        get() = when (ReflectImplementation.CURRENT) {
            ...
            ReflectImplementation.RAW_PROTOBUF_QUICKBUFFERS ->
                TODO()
        }
    ...
    override val isData: Boolean
        get() = when (ReflectImplementation.CURRENT) {
            ...
            ReflectImplementation.RAW_PROTOBUF_QUICKBUFFERS -> data.value.rawProtoBufQuick?.second?.flags?.let(Flags.IS_DATA::get) == true
        }
