...
import org.jetbrains.kotlin.metadata.jvm.QuickJvmProtoBuf.StringTableTypes.Record
import org.jetbrains.kotlin.metadata.jvm.QuickJvmProtoBuf.StringTableTypes.Record.Operation.*
import us.hebi.quickbuf.RepeatedMessage

class QuickJvmNameResolver(
    val types: QuickJvmProtoBuf.StringTableTypes,
    strings: Array<String>
) : QuickJvmNameResolverBase(
    strings,
    types.localName.toSet(),
...
private fun RepeatedMessage<Record>.toExpandedRecordsList(): List<Record> =
    ArrayList<Record>().also { list ->
        list.ensureCapacity(length())
        for (record in this as Iterable<Record>) {
            repeat(record.range) {
                list.add(record)
            }
        }
...
open class QuickJvmNameResolverBase(
    ...
    override fun getString(index: Int): String {
        ...
        if (record.substringIndex.length() >= 2) {
            val begin = record.substringIndex[0]
            val end = record.substringIndex[1]
            if (0 <= begin && begin <= end && end <= string.length) {
                string = string.substring(begin, end)
            }
        }

        if (record.replaceChar.length() >= 2) {
            val from = record.replaceChar[0]
            val to = record.replaceChar[0]
