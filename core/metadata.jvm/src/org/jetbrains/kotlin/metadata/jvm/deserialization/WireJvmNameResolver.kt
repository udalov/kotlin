...
import org.jetbrains.kotlin.metadata.jvm.WireJvmProtoBuf.StringTableTypes
import org.jetbrains.kotlin.metadata.jvm.WireJvmProtoBuf.StringTableTypes.Record
import org.jetbrains.kotlin.metadata.jvm.WireJvmProtoBuf.StringTableTypes.Record.Operation.*
import kotlin.collections.component1
import kotlin.collections.component2

class WireJvmNameResolver(
    val types: StringTableTypes,
    strings: Array<String>
) : WireJvmNameResolverBase(
    strings,
    types.local_name.run { if (isEmpty()) emptySet() else toSet() },
    types.record.toExpandedRecordsList()
)

@JvmName("wireToExpandedRecordsList")
fun List<Record>.toExpandedRecordsList(): List<Record> =
    ArrayList<Record>().also { list ->
        list.ensureCapacity(size)
        for (record in this) {
            repeat(record.range ?: 1) { // TODO: `Wire.get(record.range, 1)`?
                list.add(record)
            }
        }
        list.trimToSize()
...
open class WireJvmNameResolverBase(
    ...
    override fun getString(index: Int): String {
        ...
        var string = when {
            record.string != null -> record.string
            record.predefined_index != null && record.predefined_index in PREDEFINED_STRINGS.indices ->
                PREDEFINED_STRINGS[record.predefined_index]
            else -> strings[index]
        }

        if (record.substring_index != null && record.substring_index.size >= 2) {
            val (begin, end) = record.substring_index
            if (0 <= begin && begin <= end && end <= string.length) {
                string = string.substring(begin, end)
            }
        }

        if (record.replace_char != null && record.replace_char.size >= 2) {
            val (from, to) = record.replace_char
